export const validateForm = (data, isUpdate) => {
  const errors = {};
  ...
  if (!isUpdate && !data.photo?.size > 0)
    errors.photo = "Spot photo is required.";
  if (!data.total_visitors_per_year)
    errors.total_visitors_per_year = "Visitors count is required.";
  return errors;
};
